# -*- coding: utf-8 -*-
"""
MULTIFUNCTION WORK / PARTY ISLAND - CLOSED WORKTOP CAD V2.3
==========================================================
...
Outputs (in the repository-root ``output_v23`` directory):

    island_concept_v23.FCStd
    island_concept_v23_WORK.step
    island_concept_v23_PARTY.step
    island_concept_v23_collision_report.json
    island_concept_v23_plan.svg
...
V2.3 follows the marked continuous desk edges: P widens toward the centre and
shares its sloping sides with S/T. Monitor modules no longer pass through desk
footprints. P lifts sit behind the rear P edges and paired S/T lifts use the
400 mm technical channels. Every P/S/T desk has parallel front and rear edges,
closing the worktop-plan layout before party-layer design. It remains a concept
model, not manufacturing documentation. Hinges, locks, fasteners, actuators,
cable chains and structural sizing still need engineering.
...
import Part
import geometry_v23 as Geometry
...
DOCUMENT_NAME = "island_concept_v23"
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "output_v23")
...
def add_document_parameters(doc):
    ...
    for name, value in values:
        params.addProperty("App::PropertyLength", name, "V2.3 dimensions")
        setattr(params, name, value)
    params.addProperty("App::PropertyString", "ModelStatus", "V2.3 metadata")
    params.ModelStatus = "Closed worktop-plan concept CAD - not manufacturing documentation"
    return params
...
def write_design_reports(cad_collisions):
    ...
    if failed:
        raise RuntimeError(
            "V2.3 validation failed; see %s" % report_path
        )
...
def main():
    App.Console.PrintMessage("Building multifunction island closed-worktop CAD V2.3...\n")
    built = build_island()
    doc = built["doc"]
