"""Pure plan-geometry calculations for the MultiRoleWorkIsland V2.2 study.
...
def side_polygon(kind, station_angle):
    """Return a side desk joined to the marked sloping edge of primary P."""
    arm_delta, channel_edge, outer_edge = _side_lane(kind)
    arm_angle = station_angle + arm_delta
    primary = primary_polygon(station_angle)
    far_channel = local_to_world(ARM_OUTER_RADIUS, channel_edge, arm_angle)
    far_outer = local_to_world(ARM_OUTER_RADIUS, outer_edge, arm_angle)
    if kind == "S":
        return (primary[3], far_channel, far_outer, primary[2])
    return (primary[0], far_channel, far_outer, primary[1])
