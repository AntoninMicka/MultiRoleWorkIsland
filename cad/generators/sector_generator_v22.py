# -*- coding: utf-8 -*-
"""
MULTIFUNCTION WORK / PARTY ISLAND - MONITOR-BAY CAD V2.2
========================================================
...
Outputs (in the repository-root ``output_v22`` directory):

    island_concept_v22.FCStd
    island_concept_v22_WORK.step
    island_concept_v22_PARTY.step
    island_concept_v22_collision_report.json
    island_concept_v22_plan.svg
...
V2.2 follows the marked continuous desk edges: P widens toward the centre and
...
400 mm technical channels. It remains a concept model, not manufacturing
...
import Part
import geometry_v22 as Geometry
...
DOCUMENT_NAME = "island_concept_v22"
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "output_v22")
...
def add_document_parameters(doc):
    ...
    for name, value in values:
        params.addProperty("App::PropertyLength", name, "V2.2 dimensions")
        setattr(params, name, value)
    params.addProperty("App::PropertyString", "ModelStatus", "V2.2 metadata")
    params.ModelStatus = "Monitor-bay concept CAD - not manufacturing documentation"
    return params
...
def write_design_reports(cad_collisions):
    ...
    if failed:
        raise RuntimeError(
            "V2.2 validation failed; see %s" % report_path
        )
...
def main():
    App.Console.PrintMessage("Building multifunction island monitor-bay CAD V2.2...\n")
    built = build_island()
    doc = built["doc"]
